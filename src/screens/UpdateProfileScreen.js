...
export default function UpdateProfileScreen({ navigation, route }) {
    ...
    (async () => {
      try {
        ...
        if (userId) {
          ...
          console.log("UpdateProfile: loaded by id", userId, user);
        } else if (email) {
          user = getUserByEmail(email);
          console.log("UpdateProfile: loaded by email", email, user);
        } else {
          console.log("UpdateProfile: no userId/email passed in route.params");
        }
        ...
        console.error("UpdateProfile load error:", e);
  ...
  // rest unchanged but use userEmail instead of email variable (avoid shadowing)
  ...
  const onUpdate = async () => {
    ...
    setLoading(true);
    try {
      const ok = updateUserProfile(userId || null, {
        name: name.trim(),
        avatar_uri: avatarUri || null,
        // email is read-only, keep userEmail
        phone: phone.trim(),
